...
import { getDb, addMemory, searchMemories, removeMemory, insertCheckpoint, deleteCheckpoint, logAudit } from "../store/db.js";
...
export function createTools(deps: ToolDeps): Tool<any>[] {
  return [
      ...
      handler: async (args) => {
        ...
        for (const blocked of BLOCKED_WORKER_DIRS) {
          ...
          if (resolvedDir === blockedPath || resolvedDir.startsWith(blockedPath + sep)) {
            logAudit("worker_blocked_dir", "daemon", { workerName: args.name, dir: args.working_dir, blocked }, getCurrentSourceChannel() ?? "system");
        ...
        db.prepare(
          `INSERT OR REPLACE INTO worker_sessions (name, copilot_session_id, working_dir, status)
           VALUES (?, ?, ?, 'idle')`
        ).run(args.name, session.id, args.working_dir);

        logAudit("worker_create", "daemon", { name: args.name, workingDir: args.working_dir }, getCurrentSourceChannel() ?? "system");
        ...
        if (args.initial_prompt) {
          ...
          insertCheckpoint({
            ...
            origin_channel: worker.originChannel,
          });

          const timeoutMs = config.workerTimeoutMs;
          session.sendAndWait(`Working directory: ${args.working_dir}\n\n${args.initial_prompt}`, timeoutMs).then((result) => {
    ...
    defineTool("list_skills", {
      description:
        "List all available skills that Hoot 🦉 knows. Skills are instruction documents that teach Hoot 🦉 " +
    ...
    defineTool("learn_skill", {
      description:
        "Teach Hoot 🦉 a new skill by creating a SKILL.md instruction file. Use this when the user asks Hoot 🦉 " +
      ...
      handler: async () => {
        try {
          ...
          const lines = models.map((m) => {
            ...
            const billing = m.billingMultiplier ? ` (${m.billingMultiplier}x)` : "";
    ...
    defineTool("switch_model", {
      description:
        "Switch the Copilot model Hoot uses for conversations. Takes effect on the next message. " +
    ...
    defineTool("toggle_auto", {
      description:
        "Enable or disable automatic model routing (auto mode). When enabled, Hoot 🦉 automatically picks " +
    ...
    defineTool("remember", {
      ...
      parameters: z.object({
        ...
        source: z.enum(["user", "auto"]).optional().describe("'user' if explicitly asked to remember, 'auto' if Hoot detected it (default: 'user')"),
      }),
      handler: async (args) => {
    ...
    defineTool("recall", {
      description:
        "Search Hoot's long-term memory for stored facts, preferences, or information. " +
    ...
    defineTool("forget", {
      description:
        "Remove a specific memory from Hoot's long-term storage. Use when the user asks " +
    ...
    defineTool("restart_hoot", {
      description:
        "Restart the Hoot 🦉 daemon process. Use when the user asks Hoot 🦉 to restart itself, " +
